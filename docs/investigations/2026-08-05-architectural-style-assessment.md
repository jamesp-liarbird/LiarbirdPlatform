...
> **Provenance.** Written as a LiarbirdServer working document and moved here to be version-controlled, because that repo's `docs/investigations/` is untracked. Kept verbatim as a dated record. Two references resolve outside this repo: `ADR-006` and `ADR-007` are the two ADRs carried over as [`control-plane-tenant-plane-separation.md`](../adr/control-plane-tenant-plane-separation.md) and [`time-keyed-tables-with-bounded-retention.md`](../adr/time-keyed-tables-with-bounded-retention.md); the sibling audits it cites (`2026-08-05-endpointmgr-only-retirement-audit.md`, `2026-07-31-deployment-shapes-audit.md`) were not moved and remain uncommitted in LiarbirdServer.
